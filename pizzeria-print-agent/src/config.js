...
function parseLocalBindHost(value) {
  const normalized = String(value || "127.0.0.1").trim();
...
const localAdminToken = String(process.env.LOCAL_ADMIN_TOKEN || "").trim();
if (nodeEnv === "production" && !localAdminToken) {
  throw new Error("LOCAL_ADMIN_TOKEN is required when NODE_ENV=production");
}
...
const config = {
  ...
  localHttpHost: parseLocalBindHost(process.env.LOCAL_HTTP_HOST),
  localAdminToken,
