...
function createHealthServer({ config, agent, logger }) {
  ...
  if (!config.localAdminToken) {
    logger.warn(
      ...
      "LOCAL_ADMIN_TOKEN missing: sensitive local endpoints are unsecured outside production"
    );
  }
  ...
  const server = app.listen(config.localHttpPort, config.localHttpHost, () => {
    logger.info(
      {
        host: config.localHttpHost,
        port: config.localHttpPort,
      },
      "local health server started"
    );
  });
