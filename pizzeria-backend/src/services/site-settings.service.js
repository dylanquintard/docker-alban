...
const DEFAULT_SITE_SETTINGS = Object.freeze({
  ...
  pizzaPage: {
    sectionTitle: {
      fr: "Nos pizzas du moment",
      en: "Our featured pizzas",
    },
    sectionDescription: {
      fr: "Une sélection mise en avant par l'équipe. Commandez en ligne en quelques clics.",
      en: "A selection highlighted by the team. Order online in just a few clicks.",
    },
    featuredProductIds: [],
  },
});
...
const LOCALIZED_TRANSLATION_PATHS = [
  ...
  ["footer", "copyright"],
  ["announcement", "text"],
  ["pizzaPage", "sectionTitle"],
  ["pizzaPage", "sectionDescription"],
];
  ...
  const next = {
      ...
      source.seo !== undefined
        ? {
            ...
            defaultMetaDescription:
              ...
              faviconUrl:
                ensureObject(source.seo, "seo").faviconUrl !== undefined
                  ? normalizeString(
                      ensureObject(source.seo, "seo").faviconUrl,
                      "seo.faviconUrl"
                    )
                  : current.seo.faviconUrl,
              canonicalSiteUrl:
        ...
        ? {
            ...
            pickupConfirmationText:
              ensureObject(source.order, "order").pickupConfirmationText !== undefined
                ...
                : current.order.pickupConfirmationText,
            showMenuProductImages:
              ensureObject(source.order, "order").showMenuProductImages !== undefined
                ? Boolean(ensureObject(source.order, "order").showMenuProductImages)
                : current.order.showMenuProductImages,
          }
        : current.order,
    footer:
    ...
    announcement:
      source.announcement !== undefined
        ...
        : current.announcement,
    pizzaPage:
      source.pizzaPage !== undefined
        ? {
            ...current.pizzaPage,
            sectionTitle:
              ensureObject(source.pizzaPage, "pizzaPage").sectionTitle !== undefined
                ? mergeLocalizedValue(
                    current.pizzaPage.sectionTitle,
                    ensureObject(source.pizzaPage, "pizzaPage").sectionTitle,
                    "pizzaPage.sectionTitle"
                  )
                : current.pizzaPage.sectionTitle,
            sectionDescription:
              ensureObject(source.pizzaPage, "pizzaPage").sectionDescription !== undefined
                ? mergeLocalizedValue(
                    current.pizzaPage.sectionDescription,
                    ensureObject(source.pizzaPage, "pizzaPage").sectionDescription,
                    "pizzaPage.sectionDescription"
                  )
                : current.pizzaPage.sectionDescription,
            featuredProductIds:
              ensureObject(source.pizzaPage, "pizzaPage").featuredProductIds !== undefined
                ? Array.from(
                    new Set(
                      (Array.isArray(ensureObject(source.pizzaPage, "pizzaPage").featuredProductIds)
                        ? ensureObject(source.pizzaPage, "pizzaPage").featuredProductIds
                        : []
                      )
                        .map((value) => Number(value))
                        .filter((value) => Number.isInteger(value) && value > 0)
                ...
                : current.pizzaPage.featuredProductIds,
          }
        : current.pizzaPage,
  };
