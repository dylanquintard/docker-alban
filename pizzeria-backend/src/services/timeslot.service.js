...
function buildWeeklyServiceId({ dayOfWeek, locationId, startTime, endTime }) {
  const parsedDayOfWeek = parseDayOfWeek(dayOfWeek, "dayOfWeek");
  const parsedLocationId = parseOptionalPositiveInt(locationId, "locationId");
  const normalizedStartTime = String(startTime || "").trim();
  const normalizedEndTime = String(endTime || "").trim();

  if (!parsedLocationId || !normalizedStartTime || !normalizedEndTime) {
    return null;
  }

  const raw = `${parsedDayOfWeek}|${parsedLocationId}|${normalizedStartTime}|${normalizedEndTime}`;
  return `svc_${encodeBase64Url(raw)}`;
}

function parseWeeklyServiceId(serviceId) {
  const normalizedServiceId = String(serviceId || "").trim();
  if (!normalizedServiceId.startsWith("svc_")) {
    throw new Error("serviceId is invalid");
  }

  let decoded;
  try {
    decoded = decodeBase64Url(normalizedServiceId.slice(4));
  } catch (_err) {
    throw new Error("serviceId is invalid");
  }

  const [dayOfWeek, locationIdRaw, startTime, endTime] = decoded.split("|");
  if (!dayOfWeek || !locationIdRaw || !startTime || !endTime) {
    throw new Error("serviceId is invalid");
  }

  return {
    dayOfWeek: parseDayOfWeek(dayOfWeek, "serviceId.dayOfWeek"),
    locationId: parsePositiveInt(locationIdRaw, "serviceId.locationId"),
    startTime,
    endTime,
  };
}
  ...
  for (const groupSlots of byMergeKey.values()) {
    ...
    for (const slot of ordered) {
      ...
      if (canMerge) {
        ...
        current.serviceId = buildWeeklyServiceId({
          dayOfWeek,
      ...
      current = {
        ...
        serviceId: buildWeeklyServiceId({
          dayOfWeek,
...
async function removeWeeklyService(dayOfWeek, payload = {}) {
  ...
  const anchorDate = getAnchorDateForDay(parsedDay);
  const range = getDateRange(anchorDate);
  const serviceId = parseOptionalText(payload.serviceId, "serviceId", 500);
  ...
  if (serviceId) {
    const parsedService = parseWeeklyServiceId(serviceId);
    if (parsedService.dayOfWeek !== parsedDay) {
      throw new Error("serviceId does not match selected day");
    }
    startTime = parsedService.startTime;
    endTime = parsedService.endTime;
    locationId = parsedService.locationId;
  } else {
    locationId = parseOptionalPositiveInt(payload.locationId, "locationId");
  }
  ...
  const serviceStart = buildDateTime(anchorDate, startTime, "startTime");
  const serviceEnd = buildDateTime(anchorDate, endTime, "endTime");
  if (serviceEnd <= serviceStart) {
    throw new Error("endTime must be after startTime");
  }
