...
const STATIC_FAQ_TARGETS = Object.freeze([
  ...
  { path: "/camion-pizza-moselle", label: "Camion pizza Moselle", type: "static" },
  { path: "/menu", label: "Menu", type: "static" },
  { path: "/pizza", label: "Nos pizzas", type: "static" },
  { path: "/planing", label: "Horaires d'ouvertures", type: "static" },
]);

const LEGACY_TARGET_PATH_ALIASES = Object.freeze({
  "/food-truck-pizza-moselle": "/camion-pizza-moselle",
});
...
function normalizeTargetPath(value) {
  ...
  return LEGACY_TARGET_PATH_ALIASES[lowered] || lowered;
}

function getTargetPathCandidates(path) {
  const normalized = normalizeTargetPath(path);
  const candidates = new Set([normalized]);

  for (const [legacyPath, canonicalPath] of Object.entries(LEGACY_TARGET_PATH_ALIASES)) {
    if (canonicalPath === normalized) {
      candidates.add(legacyPath);
    }
    if (legacyPath === normalized) {
...
async function getPublicFaqEntries(path) {
  ...
  const targetPathCandidates = getTargetPathCandidates(targetPath);
  const records = await prisma.pageFaq.findMany({
    where: {
      targetPath: {
        in: targetPathCandidates,
      },
      active: true,
    },
