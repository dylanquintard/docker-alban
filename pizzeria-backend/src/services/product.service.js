...
async function clearOtherRecommendedSupplements(tx, productId, keepIngredientId) {
  const recommendedLinks = await tx.productIngredient.findMany({
    where: {
      productId,
      isRecommended: true,
      ingredientId:
        keepIngredientId === undefined
          ? undefined
          : {
              not: keepIngredientId,
            },
    },
    select: {
      id: true,
  ...
  const idsToClear = recommendedLinks
    .filter((entry) => Boolean(entry?.ingredient?.isExtra))
    .map((entry) => entry.id);
...
async function createProduct(data) {
  ...
  const product = await prisma.product.create({
    data: {
      ...
      imageAlt: parseOptionalString(data.imageAlt),
      categoryId,
    },
...
async function addIngredientToProduct(productId, ingredientId, data = {}) {
  ...
  const shouldRecommend =
    parseBooleanWithDefault(data.isRecommended, false) && Boolean(ingredient.isExtra);

  try {
    return await prisma.$transaction(async (tx) => {
      if (shouldRecommend) {
        await clearOtherRecommendedSupplements(tx, parsedProductId, parsedIngredientId);
      }

      return tx.productIngredient.create({
        data: {
          productId: parsedProductId,
          ingredientId: parsedIngredientId,
          isBase: parseBooleanWithDefault(data.isBase, false),
          isAfterCooking: parseBooleanWithDefault(data.isAfterCooking, false),
...
async function updateIngredientLinkOnProduct(productId, ingredientId, data = {}) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    return tx.productIngredient.update({
      where: {
