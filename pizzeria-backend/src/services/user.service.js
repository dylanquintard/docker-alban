...
function getPasswordResetBaseUrl(options = {}) {
  const explicitBase = String(options.passwordResetUrlBase ?? "").trim();
  if (explicitBase && /^https?:\/\//i.test(explicitBase)) {
    return explicitBase.replace(/\/+$/, "");
  }

  const frontendBase = String(options.frontendSiteUrl ?? "").trim();
  if (frontendBase) {
    return `${frontendBase.replace(/\/+$/, "")}/reset-password`;
  }

  const fallbackOrigin = String(options.corsOrigins?.[0] ?? "").trim();
  if (fallbackOrigin) {
    return `${fallbackOrigin.replace(/\/+$/, "")}/reset-password`;
  }

  const envExplicitBase = String(process.env.PASSWORD_RESET_URL_BASE ?? "").trim();
  if (envExplicitBase && /^https?:\/\//i.test(envExplicitBase)) {
    return envExplicitBase.replace(/\/+$/, "");
  }

  const envFrontendBase = String(FRONTEND_SITE_URL ?? "").trim();
  if (envFrontendBase) {
    return `${envFrontendBase.replace(/\/+$/, "")}/reset-password`;
  }

  const envFallbackOrigin = String(CORS_ORIGINS?.[0] ?? "http://localhost:3000").trim();
  return `${envFallbackOrigin.replace(/\/+$/, "")}/reset-password`;
}
